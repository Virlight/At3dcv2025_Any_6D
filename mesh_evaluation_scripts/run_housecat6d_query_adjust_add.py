...
if __name__ == '__main__':
    ...
    save_root = f"./results/housecat6d_query_results_adjust_add/{name}/{date_str}"
...
# obtain pose A->Q by pose m->A and pose m->Q
            ...
            print(f"Frame {i} - Object {obj_f}: R error: {err_R}, T error: {err_T}")
            print(f"Frame {i} - Object {obj_f}: Success Rate: {succ_pose.mean() * 100:.2f}%")
            print(f"Frame {i} - Object {obj_f}: Success R: {succ_r.mean() * 100:.2f}%, Success T: {succ_t.mean() * 100:.2f}%")
            # Compute ADD and ADD-S
            print(f"Frame {i} - Object {obj_f}: GT Pose: {gt_pose_q}")
            print(f"Frame {i} - Object {obj_f}: Pred Pose: {pred_q}")
            print(f"Frame {i} - Object {obj_f}: Pgt_mesh.vertices: {gt_mesh.vertices}")
            ...
            # EDIT: 
            object_metrics[obj_f]['ADD'].append(add)
            object_metrics[obj_f]['ADD-S'].append(adds)
        ...
        # EDIT: 
        means_all = {
            'ADD-S': np.mean(object_metrics[obj_f]['ADD-S']) ,
            'ADD': np.mean(object_metrics[obj_f]['ADD']) ,
    ...
    means_all = {
        'Frame_ID': 'MEAN',
        'Class': 'ALL',
        'ADD-S': f"{df_all_frames['ADD-S'].mean() :.1f}",
        'ADD': f"{df_all_frames['ADD'].mean() :.1f}",
