...
if __name__ == '__main__':
    ...
    output_dir = os.path.join("anchor_housecat6d/vggt_depth", os.path.basename(scene_dir), obj, frame)
    ...
    depth = cv2.imread(os.path.join(scene_dir, "depth_vggt", depth_img), cv2.IMREAD_ANYDEPTH).astype(np.float32) / 1000.0
    ...
    # Pose estimation
    pred_pose = est.register_any6d(K=intrinsic, rgb=color, depth=depth, ob_mask=mask, iteration=5, name=obj)
