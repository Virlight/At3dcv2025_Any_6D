...
if __name__=='__main__':
    ...
    parser.add_argument("--anchor_folder", type=str, default="dexycb_reference_view_preprocess", help="Path to the YCB-V model info JSON")
    ...
    for obj in tqdm(obj_list, desc='Object'):
        ...
        result_path = f'{anchor_folder}/results/{obj}'
        ...
        preprocessed_color = cv2.cvtColor(cv2.imread(os.path.join(save_path, 'input.png')), cv2.COLOR_BGR2RGB)
        ...
        if img_to_3d:
            ...
            input_image = Image.fromarray(preprocessed_color)
        ...
        np.savetxt(os.path.join(result_path, f'{obj}_initial_pose.txt'), pred_pose)
        est.mesh.export(os.path.join(result_path, f'final_mesh_{obj}.obj'))

        np.savetxt(os.path.join(result_path, f'{obj}_cd.txt'), [chamfer_dis])
