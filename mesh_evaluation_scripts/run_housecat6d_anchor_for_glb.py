...
from estimater_for_glb import Any6D
from foundationpose.Utils_for_glb_mesh_compare import get_bounding_box, vis_mask_contours, draw_xyz_axis,visualize_frame_results, calculate_chamfer_distance_gt_mesh, align_mesh_to_coordinate
...
if __name__ == '__main__':
    ...
    if args.img_to_3d:
        ...
        # mesh_path = os.path.join("/home/haoliang/Downloads", "cup_hunyuan.glb")
        ...
        # for attr in ['image', 'baseColorTexture', 'metallicRoughnessTexture', 'emissiveTexture']:
        #     tex = getattr(mesh.visual.material, attr, None)
        #     print(f"{attr}: {tex}")
        #     if tex is not None:
        #         print(f" -> image: {getattr(tex, 'image', None)}")

        # print(f"######################## Mesh Info End #########################")

    mesh = align_mesh_to_coordinate(mesh)
    if args.img_to_3d:
        mesh.export(os.path.join(output_dir, f'center_mesh_{obj}.obj'))
    else:
        mesh.export(os.path.join(output_dir, f'center_benchmark_mesh_{obj}.obj'))
