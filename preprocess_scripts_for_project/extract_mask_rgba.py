...
centered_img = np.zeros((H, W, 4), dtype=np.uint8)  # 全 0，表示完全透明
...
# 在 RGBA 图中填入 RGB 和 alpha 通道
# RGB
centered_img[y1:y1+obj_h, x1:x1+obj_w, :3][obj_mask > 0] = obj_img[obj_mask > 0]
# Alpha：掩码区域为不透明（255）
centered_img[y1:y1+obj_h, x1:x1+obj_w, 3][obj_mask > 0] = 255
