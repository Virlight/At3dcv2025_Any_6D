...
class Any6D:
    ...
    def reset_object(self, mesh=None, symmetry_tfs=None):
        ...
        if hasattr(mesh.visual, 'uv') and mesh.visual.uv is not None:
            ...
            if image is not None:
                img = copy.deepcopy(np.array(image.convert('RGB')))
                uv = copy.deepcopy(mesh.visual.uv)
                uv[:, 1] = 1 - uv[:, 1]
    ...
    def register_any6d(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5, name=None, refinement=True, axis_align=True,coarse_est=True, uniform_scale=False):
        '''Copmute pose from given pts to self.pcd
        @pts: (N,3) np array, downsampled scene points
        '''
        set_seed(0)
        ...
        if refinement:
            ...
            if True:
                ...
                if uniform_scale:
                    # 只采样一个缩放因子数组
                    uniform_scales = np.random.uniform(ratio_i, ratio_l, num_samples)
                    # 把这个缩放因子赋给x, y, z 三个轴
                    scaling_matrices = np.array([np.diag([s, s, s, 1]) for s in uniform_scales])
                ...
                print(f"scale {scale}")
                scale_items = [f"{x:.4f}" for x in scale]
                line = " ".join(scale_items)
                line = f"({line})"
                with open('selected_scale.txt', 'w') as f:
                    f.write(line + '\n')
